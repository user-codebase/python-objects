...
def get_movies(video_library):
    return sorted([item for item in video_library if isinstance(item, Video)], key=lambda item: item.title)

def get_series(video_library):
    return sorted([item for item in video_library if isinstance(item, Series)], key=lambda item: item.title)
...
def top_titles(video_library, amount, content_type):
    ...
    list_with_videos = sorted(items, key=lambda item: item.views, reverse=True)
...
if __name__ == '__main__':
    ...
    movie_1 = Video("Movie_1", 1990, "Action")
    movie_2 = Video("Movie_2", 2010, "Crime")
    movie_3 = Video("Movie_3", 2000, "Sci-Fi")
